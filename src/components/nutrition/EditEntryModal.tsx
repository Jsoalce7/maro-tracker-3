...
export function EditEntryModal({ entries, onClose, onUpdate, onDelete, onEditFoodData }: EditEntryModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center md:items-center">
            ...
            {/* Modal */}
            <div className="relative w-full max-w-md bg-[#1A1A1A] rounded-t-3xl md:rounded-3xl p-6 flex flex-col animate-slide-up shadow-2xl ring-1 ring-white/10">
                ...
                <div className="space-y-6">
                                ...
                                <div className="flex bg-[#141414] rounded-xl border border-[#2A2A2A] overflow-hidden shrink-0">
                                    {(['g', 'ml', 'oz', 'serving'] as const).map(u => (
                                        <button
                                            ...
                                            className={`px-3 py-3 font-medium text-sm transition-colors ${unit === u ? 'bg-[#3B82F6] text-white' : 'text-[#6B6B6B] hover:text-white'}`}
                                        >
                                            {u === 'serving' ? 'Srv' : u}
                                        </button>
                                    ))}
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        <div className="bg-[#141414] rounded-xl p-3 text-center">
                            ...
                            <p className="text-white font-semibold">{nutrition.calories}</p>
                        </div>
                        <div className="bg-[#141414] rounded-xl p-3 text-center">
                            <p className="text-xs text-[#EF4444] mb-1">Prot</p>
                            <p className="text-white font-semibold">{nutrition.protein}g</p>
                        </div>
                        <div className="bg-[#141414] rounded-xl p-3 text-center">
                            <p className="text-xs text-[#10B981] mb-1">Carb</p>
                            <p className="text-white font-semibold">{nutrition.carbs}g</p>
                        </div>
                        <div className="bg-[#141414] rounded-xl p-3 text-center">
                            <p className="text-xs text-[#F59E0B] mb-1">Fat</p>
                            <p className="text-white font-semibold">{nutrition.fat}g</p>
                        </div>
                    </div>
